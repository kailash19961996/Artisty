...
def create_assistant(openai_api_key: str, model_name: str = "gpt-4o-mini") -> ArtistryAssistant:
    """Create and initialize the Artisty assistant with inventory data"""
    import os
    ...
    # Load art inventory
    try:
        # Try to read art.txt from current directory first
        if os.path.exists("art.txt"):
            with open("art.txt", "r", encoding="utf-8") as f:
                inventory_text = f.read()
        # Fallback: try lambda directory
        elif os.path.exists("lambda/art.txt"):
            with open("lambda/art.txt", "r", encoding="utf-8") as f:
                inventory_text = f.read()
        # Another fallback: try parent directory
        elif os.path.exists("../art.txt"):
            with open("../art.txt", "r", encoding="utf-8") as f:
                inventory_text = f.read()
        else:
            # Use a basic fallback inventory
            inventory_text = "Art inventory file not found."
    except Exception as e:
        print(f"Warning: Could not load art inventory: {e}")
        inventory_text = "Art inventory file not found."
